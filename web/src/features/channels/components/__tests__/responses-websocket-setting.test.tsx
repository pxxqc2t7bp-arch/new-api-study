...
describe('Responses WebSocket channel setting', () => {
  ...
  test.each(supportedChannelTypes)(
    ...
    async (channelType) => {
      expect(supportsResponsesWebSocket(channelType)).toBe(true)
  ...
  test.each([14])(
    'unsupported channel type %s hides the switch and cannot serialize enabled',
    async (channelType) => {
      expect(supportsResponsesWebSocket(channelType)).toBe(false)
      const user = userEvent.setup()
      const saved: string[] = []
      render(
        <SettingsForm
