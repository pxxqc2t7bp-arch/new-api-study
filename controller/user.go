...
func GetAllUsers(c *gin.Context) {
	...
	users, total, err := model.GetAllUsersForViewer(c.GetInt("role"), pageInfo, sortOptions)
	if err != nil {
		common.ApiError(c, err)
		return
	}
...
func SearchUsers(c *gin.Context) {
	...
	pageInfo := common.GetPageQuery(c)
	sortOptions := model.NewUserSortOptions(c.Query("sort_by"), c.Query("sort_order"))
	users, total, err := model.SearchUsersForViewer(c.GetInt("role"), keyword, group, role, status, pageInfo.GetStartIdx(), pageInfo.GetPageSize(), sortOptions)
	if err != nil {
		common.ApiError(c, err)
		return
	}
