...
func TestGetRandomSatisfiedChannelVisitsEachPriorityOnce(t *testing.T) {
	...
	for retry, wantPriority := range []int64{30, 20, 10} {
		channel, err := GetRandomSatisfiedChannel("default", "model-a", retry, nil)
		require.NoError(t, err)
		require.NotNil(t, channel)
		assert.Equal(t, wantPriority, channel.GetPriority())
	}

	channel, err := GetRandomSatisfiedChannel("default", "model-a", 3, nil)
	require.NoError(t, err)
	assert.Nil(t, channel)

	count, err := CountSatisfiedChannelPriorities("default", "model-a", nil)
	require.NoError(t, err)
...
func TestGetRandomSatisfiedChannelDoesNotRepeatSinglePriority(t *testing.T) {
	...
	channel, err := GetRandomSatisfiedChannel("default", "model-a", 1, nil)
	require.NoError(t, err)
	assert.Nil(t, channel)
}
...
func TestGetChannelFiltersPathBeforeChoosingPriority(t *testing.T) {
	...
	channel, err := GetChannel("default", modelName, 0, []taskdto.ChannelFilter{{
