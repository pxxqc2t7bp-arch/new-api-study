...
func requireHandledPlanQuotaDisable(
	t *testing.T,
	...
	t.Helper()
	result, err := DisableChannelForAPIError(channelError, observedTag, apiError)
	require.NoError(t, err)
...
func TestDisableChannelForAPIErrorHonorsGlobalAutomaticDisable(t *testing.T) {
	...
	result, disableErr := DisableChannelForAPIError(types.ChannelError{
		ChannelId:   channels[0].Id,
...
func TestDisableChannelForAPIErrorUsesObservedCredentialAfterRotation(t *testing.T) {
	...
	assert.False(t, result.ChannelDisabled)
	var stored []model.Channel
	require.NoError(t, db.Order("id").Find(&stored).Error)
...
func TestDisableChannelForAPIErrorReportsOnlyNewSingleKeySourceDisable(t *testing.T) {
	for _, testCase := range []struct {
		name string
		key  string
	}{
		{name: "shared credential", key: "repeat-source-credential"},
		{name: "empty credential"},
	} {
		t.Run(testCase.name, func(t *testing.T) {
			db := setupPlanQuotaDomainTest(t)
			autoBan := 1
			tag := "plan:support:repeat-" + strings.ReplaceAll(testCase.name, " ", "-")
			channel := model.Channel{
				Name: "repeat-" + testCase.name, Key: testCase.key,
			...
			channelError := types.ChannelError{
				...
				AutoBan: true, UsingKey: testCase.key,
			}

			first, err := DisableChannelForAPIError(channelError, tag, apiError)
			require.NoError(t, err)
			require.True(t, first.Handled)
			require.True(t, first.ChannelDisabled)

			second, err := DisableChannelForAPIError(channelError, tag, apiError)
			require.NoError(t, err)
...
func TestDisableChannelForAPIErrorDoesNotReportAlreadyDisabledMultiKeyChannel(t *testing.T) {
	db := setupPlanQuotaDomainTest(t)
	autoBan := 1
	tag := "plan:support:already-disabled-multi-key"
	channel := model.Channel{
		Name: "already-disabled-multi-key", Key: "key-a\nkey-b",
	...
	result, err := DisableChannelForAPIError(types.ChannelError{
		ChannelId: channel.Id, ChannelName: channel.Name,
		IsMultiKey: true, AutoBan: true, UsingKey: "key-a",
	}, tag, apiError)
	...
	var stored model.Channel
	require.NoError(t, db.First(&stored, channel.Id).Error)
	resetAt, matched := ParsePlanQuotaReset(apiError.Error())
...
func TestDisableChannelWithResultReturnsManagedPersistenceError(t *testing.T) {
	db := setupPlanQuotaDomainTest(t)
	...
	autoBan := 1
	channel := model.Channel{
		Name: "managed-query-error", Key: "credential",
	...
	const callbackName = "test:managed_failure_query_error"
	forcedErr := errors.New("forced managed route query failure")
	require.NoError(t, db.Callback().Query().Before("gorm:query").Register(callbackName, func(tx *gorm.DB) {
		if tx.Statement != nil &&
			tx.Statement.Schema != nil &&
			tx.Statement.Schema.Name == "UpstreamManagedRoute" {
	...
	disabled, err := DisableChannelWithResult(types.ChannelError{
		ChannelId: channel.Id, ChannelName: channel.Name,
		AutoBan: true, UsingKey: channel.Key,
	}, "managed failure")
...
func TestDisableChannelWithResultReportsManagedQuarantine(t *testing.T) {
	db := setupPlanQuotaDomainTest(t)
	orchestration := operation_setting.GetUpstreamOrchestrationSetting()
	originalOrchestration := *orchestration
	orchestration.Enabled = true
	...
	autoBan := 1
	channel := model.Channel{
		Name: "managed-quarantine", Key: "credential",
		Status: common.ChannelStatusEnabled, AutoBan: &autoBan,
		Models: "gpt-3.5-turbo", Group: "default",
	}
	require.NoError(t, db.Create(&channel).Error)
	...
	require.NoError(t, db.Create(&model.UpstreamManagedRoute{
		SourceID:        1,
		ExternalGroupID: "managed-quarantine",
		Platform:        "openai",
		Protocol:        model.UpstreamProtocolOpenAI,
		ChannelID:       channel.Id,
		State:           model.UpstreamRouteStateActive,
...
func TestEnableChannelRestoresFinalMultiKeyMemoryRouting(t *testing.T) {
	...
	requireHandledPlanQuotaDisable(t, types.ChannelError{
		ChannelId:   channel.Id,
		ChannelName: channel.Name,
		IsMultiKey:  true,
		AutoBan:     true,
...
func TestDisableChannelRefreshesEmptyCredentialPlanQuotaGenerationWithCache(t *testing.T) {
	...
	requireHandledPlanQuotaDisable(t, channelError, tag, apiError)
...
func TestDisableEmptyCredentialPlanQuotaSourceNotifiesOnlyNewDisable(t *testing.T) {
	db := setupPlanQuotaDomainTest(t)
	...
	require.NoError(t, err)
	root := model.User{
		Username: "empty-plan-notification-root",
	...
	autoBan := 1
	tag := "plan:support:empty-notification"
	channel := model.Channel{
		Name: "empty-notification", Key: "",
		Status: common.ChannelStatusEnabled, Tag: &tag, AutoBan: &autoBan,
		Models: "gpt-3.5-turbo", Group: "default",
	}
	require.NoError(t, db.Create(&channel).Error)
	...
	notificationType := "channel_plan_quota_" + tag
	notifyKey := fmt.Sprintf("%d:%s:%s", root.Id, notificationType, time.Now().Format("2006010215"))
	notifyLimitStore.Delete(notifyKey)
	t.Cleanup(func() { notifyLimitStore.Delete(notifyKey) })

	const reason = "empty credential quota exhausted"
	require.NoError(t, disableEmptyCredentialPlanQuotaSource(
		channel.Id,
		tag,
		reason,
		2_000_000_000,
	...
	case <-time.After(time.Second):
		t.Fatal("empty-credential Plan quota disable notification was not delivered")
	}
	assert.Equal(t, notificationType, notification.Type)
	assert.Equal(t, fmt.Sprintf("Plan 配额域「%s」已临时禁用", tag), notification.Title)
	assert.Equal(t, reason+"，共禁用 1 个协议渠道", notification.Content)
	...
	select {
	case <-notifications:
		t.Fatal("repeat empty-credential Plan quota disable emitted a notification")
	default:
	}
}
...
func TestDisableChannelPlanMultiKeyFinalKeyWithoutKnownResetClearsStaleDeadline(t *testing.T) {
	...
	firstResult, err := DisableChannelForAPIError(channelError, tag, knownResetError)
	require.NoError(t, err)
	require.True(t, firstResult.Handled)
	require.True(t, firstResult.ChannelDisabled)
...
func TestDisablePlanQuotaDomainRollsBackAllSiblingsOnAbilityFailure(t *testing.T) {
	...
	disableErr := disablePlanQuotaDomainWithCredential(
		&channels[0],
		channels[0].Key,
		firstTag,
		"quota exhausted",
	...
	require.ErrorIs(t, disableErr, forcedErr)
