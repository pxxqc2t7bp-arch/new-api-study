...
func newResponsesWSTestRunner(t *testing.T, token *model.Token, extraHeaders ...http.Header) (relay.ResponsesWSRequestRunner, *http.Request) {
	t.Helper()
...
func TestResponsesWSRequestRunnerAppliesOrdinaryRequestPolicy(t *testing.T) {
	t.Run("authorized policy headers reach the request handler", func(t *testing.T) {
		_, token := setupResponsesWSRequestTest(t)
		require.NoError(t, model.DB.Model(token).Updates(map[string]any{
			"default_routing_strategy":   string(hosttypes.RoutingStrategyStable),
			"allowed_routing_strategies": `["stable","economy"]`,
			"default_conversion_policy":  string(types.ConversionLossPolicyStrict),
			"allow_lossy_conversion":     true,
		}).Error)
		runner, _ := newResponsesWSTestRunner(t, token, http.Header{
			middleware.HeaderRoutingStrategy:  []string{string(hosttypes.RoutingStrategyEconomy)},
			middleware.HeaderConversionPolicy: []string{string(types.ConversionLossPolicySafe)},
		})
		called := false
		apiErr := runner(httptest.NewRequest(http.MethodPost, "/v1/responses", nil), "policy-allowed", func(c *gin.Context) *types.NewAPIError {
			called = true
			routing, ok := common.GetContextKeyType[hosttypes.RoutingStrategy](c, constant.ContextKeyRoutingStrategy)
			require.True(t, ok)
			assert.Equal(t, hosttypes.RoutingStrategyEconomy, routing)
			conversion, ok := common.GetContextKeyType[types.ConversionLossPolicy](c, constant.ContextKeyConversionPolicy)
			require.True(t, ok)
			assert.Equal(t, types.ConversionLossPolicySafe, conversion)
			return nil
		})
	...
	for _, tc := range []struct {
		...
		{
			name: "unauthorized routing strategy",
			headers: http.Header{
				middleware.HeaderRoutingStrategy: []string{string(hosttypes.RoutingStrategyEconomy)},
			},
		},
		{
			name: "unauthorized conversion policy",
			headers: http.Header{
				middleware.HeaderConversionPolicy: []string{string(types.ConversionLossPolicyAllow)},
			},
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			_, token := setupResponsesWSRequestTest(t)
			runner, _ := newResponsesWSTestRunner(t, token, tc.headers)
			called := false
			apiErr := runner(httptest.NewRequest(http.MethodPost, "/v1/responses", nil), "policy-denied", func(*gin.Context) *types.NewAPIError {
				called = true
				return nil
			})
	...
	t.Run("strict session policy survives the internal request boundary", func(t *testing.T) {
		_, token := setupResponsesWSRequestTest(t)
		...
		*affinity = operation_setting.ChannelAffinitySetting{
			...
			Rules: []operation_setting.ChannelAffinityRule{{
				...
				ModelRegex:  []string{"^policy-model$"},
				PathRegex:   []string{"^/v1/responses$"},
				KeySources:  []operation_setting.ChannelAffinityKeySource{{Type: "request_header", Key: "X-Session"}},
				SessionMode: "inherit",
			}},
		}
		runner, _ := newResponsesWSTestRunner(t, token, http.Header{"X-Session": []string{"session-1"}})
		apiErr := runner(httptest.NewRequest(http.MethodPost, "/v1/responses", nil), "policy-strict", func(c *gin.Context) *types.NewAPIError {
			_, _ = service.GetPreferredChannelByAffinity(c, "policy-model", "default")
			decision := service.DecideRelayRetry(
				c,
				types.NewOpenAIError(errors.New("upstream failed"), types.ErrorCodeBadResponseStatusCode, http.StatusTooManyRequests),
				1,
			)
			assert.Equal(t, service.PolicyDecision{Action: "stop", Reason: "strict_session", Source: "global"}, decision)
			return nil
		})
		require.Nil(t, apiErr)
	})
...
func newResponsesWSBillingTest(t *testing.T, expression string, handle func(*websocket.Conn, *http.Request), httpEvents ...string) *responsesWSBillingTest {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if fixture.beforeUpgrade != nil {
			fixture.beforeUpgrade()
		}
...
func TestResponsesWSCloseRegisteredBeforeUpstreamDialCompletes(t *testing.T) {
	...
	fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0.002))`, func(ws *websocket.Conn, _ *http.Request) {
		...
		_ = ws.WriteMessage(websocket.TextMessage, []byte(`{"type":"response.created","response":{"id":"survived","status":"in_progress"}}`))
	})
	fixture.beforeUpgrade = func() {
		close(dialStarted)
		<-releaseDial
	}

	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(
		`{"type":"response.create","event_id":"close-during-dial","model":"ws-billing","input":"hi"}`,
	)))
	select {
	case <-dialStarted:
	case <-time.After(3 * time.Second):
	...
	closed := wsmanager.CloseChannel(fixture.channel.Id, "channel disabled")
	close(releaseDial)

	assert.Equal(t, 1, closed, "selected channel must be registered before upstream dial completes")
	require.NoError(t, fixture.client.SetReadDeadline(time.Now().Add(3*time.Second)))
	_, _, err := fixture.client.ReadMessage()
	assert.Error(t, err, "a close published during dial must prevent the upstream connection from surviving")
	fixture.closeAndWait(t)
	assert.Zero(t, wsmanager.CloseChannel(fixture.channel.Id, "stale registration"))
}

func TestResponsesWSNormalCompletionUnregistersChannelClose(t *testing.T) {
	fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0.002))`, func(ws *websocket.Conn, _ *http.Request) {
		if _, _, err := ws.ReadMessage(); err != nil {
			return
		}
		_ = ws.WriteMessage(websocket.TextMessage, []byte(
			`{"type":"response.completed","response":{"id":"completed","status":"completed","usage":{"input_tokens":1000,"output_tokens":1,"total_tokens":1001}}}`,
		))
	})
	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(
		`{"type":"response.create","model":"ws-billing","input":"hi"}`,
	)))
...
func TestResponsesWSDialFailureUnregistersChannelClose(t *testing.T) {
	...
	require.NoError(t, model.DB.Model(fixture.channel).Update("base_url", unreachableBaseURL).Error)

	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(
		`{"type":"response.create","event_id":"dial-failure","model":"ws-billing","input":"hi"}`,
	)))
...
func TestResponsesHTTPRetriesRetryable500WithDefaultBudget(t *testing.T) {
	fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0.002))`, func(*websocket.Conn, *http.Request) {})
	oldRetries := common.RetryTimes
	common.RetryTimes = 1
	t.Cleanup(func() { common.RetryTimes = oldRetries })
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		if attempts.Add(1) == 1 {
			...
			_, _ = fmt.Fprint(w, `{"error":{"type":"server_error","code":"server_error","message":"retry me"}}`)
			return
		}
		_, _ = fmt.Fprint(w, `{"id":"completed","status":"completed","usage":{"input_tokens":1000,"output_tokens":1,"total_tokens":1001}}`)
	}))
...
func TestResponsesWebSocketManagedDialFailoverUsesAdaptiveBudget(t *testing.T) {
	for _, tc := range []struct {
		name          string
		budgetSeconds int
		...
		{name: "visits distinct managed sources and priorities", budgetSeconds: 10, wantAttempts: 3},
		{name: "stops after the first attempt exhausts the budget", budgetSeconds: 1, firstDelay: 1100 * time.Millisecond, wantAttempts: 1},
	} {
		t.Run(tc.name, func(t *testing.T) {
			fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0.002))`, func(*websocket.Conn, *http.Request) {})
			...
			priorityHigh, priorityLow := int64(10), int64(0)
			channels := []*model.Channel{fixture.channel}
			fixture.channel.BaseURL = &servers[0].URL
			fixture.channel.Priority = &priorityHigh
			fixture.channel.AutoBan = common.GetPointer(0)
			fixture.channel.Key = "source-0-key"
			...
			for index := 1; index < 3; index++ {
				...
				channel := &model.Channel{
					Name: "managed-ws-" + fmt.Sprint(index), Key: fmt.Sprintf("source-%d-key", index),
					Status: common.ChannelStatusEnabled, Type: constant.ChannelTypeOpenAI, Group: "default",
					Models: "ws-billing", BaseURL: &servers[index].URL, Priority: priority, AutoBan: common.GetPointer(0),
				}
				channel.SetSetting(dto.ChannelSettings{ResponsesWebSocketEnabled: true})
				require.NoError(t, model.DB.Create(channel).Error)
				require.NoError(t, model.DB.Create(&model.Ability{ChannelId: channel.Id, Model: "ws-billing", Group: "default", Enabled: true}).Error)
			...
			for index, channel := range channels {
				require.NoError(t, model.DB.Create(&model.UpstreamManagedRoute{
					SourceID: int64(index + 1), ExternalGroupID: fmt.Sprintf("group-%d", index),
			...
			if tc.wantAttempts == 3 {
				for index := range attemptsBySource {
					assert.Equal(t, int64(1), attemptsBySource[index].Load(), "source %d must be attempted exactly once", index)
				}
			}
		})
	}
...
func TestResponsesWebSocketDropsResponseFramesDuringIdleGap(t *testing.T) {
	releaseLateFrames := make(chan struct{})
	fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0.002))`, func(ws *websocket.Conn, _ *http.Request) {
		if _, _, err := ws.ReadMessage(); !assert.NoError(t, err) {
			return
		}
		for _, event := range []string{
			`{"type":"response.created","stream_id":"first-stream","response":{"id":"first","status":"in_progress"}}`,
			`{"type":"response.completed","stream_id":"first-stream","response":{"id":"first","status":"completed","usage":{"input_tokens":1000,"output_tokens":10,"total_tokens":1010}}}`,
		} {
			if !assert.NoError(t, ws.WriteMessage(websocket.TextMessage, []byte(event))) {
				return
			}
		...
		<-releaseLateFrames
		for _, event := range []string{
			`{"type":"response.completed","stream_id":"first-stream","response":{"id":"first","status":"completed"}}`,
			`{"type":"error","response_id":"first","status":500,"error":{"type":"server_error","code":"server_error","message":"late response failure"}}`,
			`{"type":"error","stream_id":"first-stream","status":500,"error":{"type":"server_error","code":"server_error","message":"late stream failure"}}`,
		...
		for _, event := range []string{
			`{"type":"response.created","stream_id":"second-stream","response":{"id":"second","status":"in_progress"}}`,
			`{"type":"response.completed","stream_id":"second-stream","response":{"id":"second","status":"completed","usage":{"input_tokens":1000,"output_tokens":10,"total_tokens":1010}}}`,
		} {
			if !assert.NoError(t, ws.WriteMessage(websocket.TextMessage, []byte(event))) {
				return
			}
	...
	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(
		`{"type":"response.create","stream_id":"first-stream","model":"ws-billing","input":"first"}`,
	)))
	assert.Equal(t, "response.created", readResponsesWSTestEvent(t, fixture.client)["type"])
	assert.Equal(t, "response.completed", readResponsesWSTestEvent(t, fixture.client)["type"])
	close(releaseLateFrames)

	sessionEvent := readResponsesWSTestEvent(t, fixture.client)
	assert.Equal(t, "session.updated", sessionEvent["type"],
		"response-scoped frames from the settled request must not cross the idle gap: %v", sessionEvent)

	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(
		`{"type":"response.create","stream_id":"second-stream","model":"ws-billing","input":"second"}`,
	)))
	assert.Equal(t, "response.created", readResponsesWSTestEvent(t, fixture.client)["type"])
	assert.Equal(t, "response.completed", readResponsesWSTestEvent(t, fixture.client)["type"])
	fixture.closeAndWait(t)
...
func TestResponsesWebSocketDoesNotDropErrorForReusedStreamID(t *testing.T) {
	fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0))`, func(ws *websocket.Conn, _ *http.Request) {
		if _, _, err := ws.ReadMessage(); !assert.NoError(t, err) {
			return
		}
		for _, event := range []string{
			`{"type":"response.created","stream_id":"shared-stream","response":{"id":"first","status":"in_progress"}}`,
			`{"type":"response.completed","stream_id":"shared-stream","response":{"id":"first","status":"completed","usage":{"input_tokens":1000,"output_tokens":10,"total_tokens":1010}}}`,
		} {
			if !assert.NoError(t, ws.WriteMessage(websocket.TextMessage, []byte(event))) {
				return
			}
		...
		_ = ws.WriteMessage(websocket.TextMessage, []byte(
			`{"type":"error","stream_id":"shared-stream","status":400,"error":{"type":"invalid_request_error","code":"invalid_input","message":"second rejected"}}`,
		))
		_, _, _ = ws.ReadMessage()
	})

	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(
		`{"type":"response.create","stream_id":"shared-stream","model":"ws-billing","input":"first"}`,
	)))
	assert.Equal(t, "response.created", readResponsesWSTestEvent(t, fixture.client)["type"])
	assert.Equal(t, "response.completed", readResponsesWSTestEvent(t, fixture.client)["type"])
	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(
		`{"type":"response.create","event_id":"second-create","stream_id":"shared-stream","model":"ws-billing","input":"second"}`,
	)))
	rejection := readResponsesWSTestEvent(t, fixture.client)
	assert.Equal(t, "error", rejection["type"])
	assert.Equal(t, "second-create", rejection["event_id"])
	assert.Equal(t, "shared-stream", rejection["stream_id"])
	fixture.closeAndWait(t)
}

func TestResponsesWebSocketClosesOnUnattributedErrorDuringIdleGap(t *testing.T) {
	for _, tc := range []struct {
		name  string
		event string
	}{
		{name: "without ID", event: `{"type":"error","status":500,"error":{"type":"server_error","code":"server_error","message":"late failure"}}`},
		{name: "unknown response ID", event: `{"type":"error","response_id":"unknown","status":500,"error":{"type":"server_error","code":"server_error","message":"late failure"}}`},
		{name: "unknown stream ID", event: `{"type":"error","stream_id":"unknown-stream","status":500,"error":{"type":"server_error","code":"server_error","message":"late failure"}}`},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Equal(t, "response.created", readResponsesWSTestEvent(t, fixture.client)["type"])
			assert.Equal(t, "response.completed", readResponsesWSTestEvent(t, fixture.client)["type"])
			close(releaseLateError)

			require.NoError(t, fixture.client.SetReadDeadline(time.Now().Add(3*time.Second)))
			_, _, err := fixture.client.ReadMessage()
			require.Error(t, err, "an unattributed idle error must close instead of becoming another terminal")
			var timeout net.Error
			assert.False(t, errors.As(err, &timeout) && timeout.Timeout(), "the gateway must close before the read deadline")
			fixture.closeAndWait(t)
			assertResponsesWSAccounting(t, fixture, []int{1000})
		})
	}
...
func TestResponsesWebSocketClosesAndRefundsUnattributedErrorBeforeSecondAcceptance(t *testing.T) {
	tokenID := make(chan int, 1)
	secondReserved := make(chan int, 1)
	...
	fixture := newResponsesWSBillingTest(t, `tier("request", fixed(0.002))`, func(ws *websocket.Conn, _ *http.Request) {
		...
		for _, event := range []string{
			...
			`{"type":"response.completed","response":{"id":"first","status":"completed","usage":{"input_tokens":1000,"output_tokens":10,"total_tokens":1010}}}`,
		} {
			if !assert.NoError(t, ws.WriteMessage(websocket.TextMessage, []byte(event))) {
				return
			}
		...
		secondReserved <- token.RemainQuota
		<-releaseError
		for _, event := range []string{
			`{"type":"error","status":500,"message":"late failure"}`,
			`{"type":"response.created","response":{"id":"second","status":"in_progress"}}`,
			`{"type":"response.completed","response":{"id":"second","status":"completed","usage":{"input_tokens":1000,"output_tokens":10,"total_tokens":1010}}}`,
		} {
	...
	tokenID <- fixture.token.Id
	updates := make(chan struct{}, 4)
	const callback = "responses-ws-second-request-refund"
	...
	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(
		`{"type":"response.create","model":"ws-billing","input":"first"}`,
	)))
	assert.Equal(t, "response.created", readResponsesWSTestEvent(t, fixture.client)["type"])
	assert.Equal(t, "response.completed", readResponsesWSTestEvent(t, fixture.client)["type"])
	for len(updates) > 0 {
	...
	require.NoError(t, fixture.client.WriteMessage(websocket.TextMessage, []byte(
		`{"type":"response.create","event_id":"second-create","stream_id":"second-stream","model":"ws-billing","input":"second"}`,
	)))
	select {
	case balance := <-secondReserved:
		assert.Equal(t, 1000, balance, "the second request must commit its reservation before the error is released")
	case <-time.After(3 * time.Second):
		t.Fatal("upstream did not observe the second request reservation")
	}
	select {
	case <-updates:
	case <-time.After(3 * time.Second):
		t.Fatal("the second request reservation did not produce a committed token update")
	}
	close(releaseError)

	rejection := readResponsesWSTestEvent(t, fixture.client)
	assert.Equal(t, "error", rejection["type"])
	assert.Equal(t, float64(http.StatusInternalServerError), rejection["status"])
	assert.Equal(t, "second-create", rejection["event_id"])
	assert.Equal(t, "second-stream", rejection["stream_id"])
	rejectionError, _ := rejection["error"].(map[string]any)
	assert.Equal(t, "invalid_request_error", rejectionError["type"])
	assert.Equal(t, "late failure", rejectionError["message"])

	_, _, err := fixture.client.ReadMessage()
	require.Error(t, err, "the ambiguous second request must close before a real response can leak")
	var timeout net.Error
	assert.False(t, errors.As(err, &timeout) && timeout.Timeout(), "the gateway must close before the read deadline")
	deadline := time.NewTimer(3 * time.Second)
	defer deadline.Stop()
	for {
		require.NoError(t, model.DB.First(fixture.token, fixture.token.Id).Error)
		require.NoError(t, model.DB.First(fixture.user, fixture.user.Id).Error)
		if fixture.token.RemainQuota == 2000 && fixture.user.Quota == 99000 {
			break
		}
		select {
		case <-updates:
		case <-deadline.C:
			t.Fatal("the second request refund did not commit before test cleanup")
		}
	}
	fixture.closeAndWait(t)
	assertResponsesWSAccounting(t, fixture, []int{1000})
