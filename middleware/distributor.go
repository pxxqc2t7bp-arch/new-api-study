	...
	return func(c *gin.Context) {
		...
		if pinned || shouldSelectChannel {
			...
			if selectErr != nil {
				...
				code := selectErr.Code
				if selectErr.FilterKind != "" {
					code = channelFilterErrorCode(selectErr.FilterKind)
				} else if selectErr.NoAvailableChannel {
					code = noAvailableChannelErrorCode(constraints)
				}
