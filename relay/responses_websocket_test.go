...
func TestResponsesWSRunCallRecordsRequestPolicyTermination(t *testing.T) {
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", nil)
	session := &responsesWSSession{}
	apiErr := session.runCall(c, &responsesWSCallState{}, responsesWSCreateRequest{})
	...
	assert.Equal(t, http.StatusBadRequest, events[0].Status)
}
...
func TestResponsesWSAdaptiveSelectionSkipsHigherPriorityHTTPOnlyChannel(t *testing.T) {
	require.NoError(t, i18n.Init())
	database := setupRelayChannelDB(t)
	require.NoError(t, database.AutoMigrate(&model.Ability{}))
	priorityHigh, priorityLow := int64(10), int64(0)
	httpOnly := &model.Channel{
		Name: "http-only", Key: "sk-http", Status: common.ChannelStatusEnabled, Type: constant.ChannelTypeOpenAI,
		Group: "default", Models: "ws-model", Priority: &priorityHigh,
	}
	websocketEnabled := &model.Channel{
		Name: "websocket", Key: "sk-ws", Status: common.ChannelStatusEnabled, Type: constant.ChannelTypeOpenAI,
		Group: "default", Models: "ws-model", Priority: &priorityLow,
	}
	websocketEnabled.SetSetting(dto.ChannelSettings{ResponsesWebSocketEnabled: true})
	for _, channel := range []*model.Channel{httpOnly, websocketEnabled} {
		require.NoError(t, database.Create(channel).Error)
		require.NoError(t, database.Create(&model.Ability{
			ChannelId: channel.Id, Model: "ws-model", Group: "default", Enabled: true, Priority: channel.Priority,
		}).Error)
	}
	previousCache := common.MemoryCacheEnabled
	common.MemoryCacheEnabled = true
	...
	retry := &service.RetryParam{
		Ctx: c, ModelName: "ws-model", TokenGroup: "default", RequestPath: c.Request.URL.Path, Retry: common.GetPointer(0),
	}
	maxRetries := responsesWSRetryTimes(retry, orchestration.Enabled)

	channel, apiErr := selectResponsesWSChannel(c, "ws-model", retry)
	require.Nil(t, apiErr)
	require.NotNil(t, channel)
	assert.Zero(t, maxRetries)
	assert.Equal(t, websocketEnabled.Id, channel.Id)
	assert.Equal(t, []int64{priorityLow}, retry.PriorityPath)
	filterCount := 0
	for _, filter := range service.GetChannelConstraints(c).Filters {
		if filter.Kind == appdto.FilterResponsesWebSocket {
			filterCount++
		}
	}
...
func TestResponsesWSRegistrationRejectsDisabledChannel(t *testing.T) {
	database := setupRelayChannelDB(t)
	channel := &model.Channel{
		Name:   "disabled-responses-websocket",
		Status: common.ChannelStatusManuallyDisabled,
	}
	require.NoError(t, database.Create(channel).Error)
	client, downstream, cleanup := newTestWebSocketPair(t)
	defer cleanup()
	ctx, cancel := context.WithCancel(context.Background())
	session := &responsesWSSession{ctx: ctx, cancel: cancel, client: downstream}

	err := session.registerChannelClose(channel.Id)

	require.Error(t, err)
	require.NoError(t, client.SetReadDeadline(time.Now().Add(time.Second)))
	_, _, readErr := client.ReadMessage()
	assert.Error(t, readErr, "a channel disabled before registration must close the client without dialing upstream")
	assert.Zero(t, wsmanager.CloseChannel(channel.Id, "stale registration"))
}

func TestRealtimeWSRejectsChannelDisabledBeforeRegistration(t *testing.T) {
	database := setupRelayChannelDB(t)
	const channelID = 51000
	require.NoError(t, database.Create(&model.Channel{
		Id:     channelID,
		Name:   "disabled-realtime-websocket",
		Status: common.ChannelStatusManuallyDisabled,
	}).Error)
	client, downstream, cleanupClient := newTestWebSocketPair(t)
	defer cleanupClient()

	var dialCount atomic.Int32
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		dialCount.Add(1)
		http.Error(w, "must not dial", http.StatusServiceUnavailable)
	}))
	...
	common.SetContextKey(c, constant.ContextKeyOriginalModel, "gpt-realtime")
	info := relaycommon.GenRelayInfoWs(c, downstream)
	...
	assert.Zero(t, wsmanager.CloseChannel(channelID, "stale registration"))
}

func TestRealtimeWSCloseRegisteredBeforeUpstreamDialCompletes(t *testing.T) {
	const channelID = 51001
	database := setupRelayChannelDB(t)
	require.NoError(t, database.Create(&model.Channel{
		Id:     channelID,
		Name:   "realtime-race",
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		close(dialStarted)
		<-releaseDial
		target, err := upgrader.Upgrade(w, r, nil)
		if err != nil {
			close(upstreamDone)
			return
		}
		...
		_ = target.WriteMessage(websocket.TextMessage, []byte(`{"type":"session.created"}`))
	}))
	defer upstream.Close()
	...
	closed := wsmanager.CloseChannel(channelID, "channel disabled")
	close(releaseDial)
	...
	case <-relayDone:
	case <-time.After(3 * time.Second):
		t.Fatal("realtime relay did not exit after the pending close")
	}
	select {
	case <-upstreamDone:
	case <-time.After(3 * time.Second):
		t.Fatal("upstream connection did not close after dial release")
	}
	assert.Zero(t, wsmanager.CloseChannel(channelID, "stale registration"))
}

func TestRealtimeWSNormalCompletionUnregistersChannelClose(t *testing.T) {
	const channelID = 51002
	database := setupRelayChannelDB(t)
	require.NoError(t, database.Create(&model.Channel{
		Id:     channelID,
		Name:   "realtime-normal",
		Status: common.ChannelStatusEnabled,
	}).Error)
	previousLogConsumeEnabled := common.LogConsumeEnabled
	common.LogConsumeEnabled = false
	...
	require.NoError(t, err)
	assert.JSONEq(t, `{"type":"session.created"}`, string(message))
	select {
	case <-relayDone:
	case <-time.After(3 * time.Second):
		t.Fatal("realtime relay did not exit after normal upstream completion")
	}
	assert.Zero(t, wsmanager.CloseChannel(channelID, "stale registration"))
}
...
func TestResponsesWSSmartRouterActualModelSettlement(t *testing.T) {
	...
	require.NoError(t, ratio_setting.UpdateModelRatioByJSONString(
		`{"actual-routed-model":0.5,"alternate-routed-model":0.75,"doubao-smart-router-250928":3}`,
	))
	require.NoError(t, ratio_setting.UpdateCompletionRatioByJSONString(
		`{"actual-routed-model":4,"alternate-routed-model":2,"doubao-smart-router-250928":5}`,
	))
	...
	for _, tc := range []struct {
		...
		wantBilledModel string
		wantHealthKey   string
		wantHealthValue any
	}{
		{
			name:            "priced actual model",
			originModel:     "doubao-smart-router-250928",
			responseModels:  []string{"actual-routed-model"},
			wantActual:      "actual-routed-model",
			wantModelRatio:  0.5,
			wantCompletion:  4,
			wantBilledModel: "actual-routed-model",
		},
		{
			name:            "missing actual model",
			originModel:     "doubao-smart-router-250928",
			responseModels:  []string{"doubao-smart-router-250928"},
			wantModelRatio:  3,
			wantCompletion:  5,
			wantHealthKey:   "actual_model_missing",
			wantHealthValue: true,
		},
		{
			name:            "unpriced actual model",
			originModel:     "doubao-smart-router-250928",
			responseModels:  []string{"ws-unpriced-actual-model"},
			wantActual:      "ws-unpriced-actual-model",
			wantModelRatio:  3,
			wantCompletion:  5,
			wantHealthKey:   "actual_model_unpriced",
			wantHealthValue: "ws-unpriced-actual-model",
		},
		{
			name:            "conflicting actual models",
			originModel:     "doubao-smart-router-250928",
			responseModels:  []string{"actual-routed-model", "alternate-routed-model"},
			wantActual:      "actual-routed-model",
			wantConflict:    true,
			wantModelRatio:  3,
			wantCompletion:  5,
			wantHealthKey:   "actual_model_conflict",
			wantHealthValue: true,
		},
		{
			name:           "ordinary model unchanged",
			originModel:    "ordinary-model",
			responseModels: []string{"actual-routed-model"},
			wantModelRatio: 7,
			wantCompletion: 8,
		},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", nil)
			c.Set("group", "default")
			request := &dto.OpenAIResponsesRequest{Model: tc.originModel}
			info := &relaycommon.RelayInfo{
				OriginModelName: tc.originModel,
				UserGroup:       "default",
				UsingGroup:      "default",
				Request:         request,
			...
			if tc.originModel == "doubao-smart-router-250928" {
				info.PriceData.ModelRatio = 3
				info.PriceData.CompletionRatio = 5
			}
			accumulator := service.NewResponsesUsageAccumulator(info)
			for _, modelName := range tc.responseModels {
				accumulator.Observe(&dto.ResponsesStreamResponse{
					Type:     "response.completed",
					Response: &dto.OpenAIResponsesResponse{Model: modelName},
				})
			}

			ConsumeResponsesQuota(c, info, accumulator.Finish())

			assert.Equal(t, tc.wantActual, info.ActualUpstreamModelName)
			assert.Equal(t, tc.wantConflict, info.ActualModelConflict)
			assert.Equal(t, tc.wantModelRatio, info.PriceData.ModelRatio)
			assert.Equal(t, tc.wantCompletion, info.PriceData.CompletionRatio)
			assert.Equal(t, tc.wantBilledModel, c.GetString("billed_model"))
			for _, key := range []string{"actual_model_missing", "actual_model_unpriced", "actual_model_conflict"} {
				value, exists := c.Get(key)
				if key == tc.wantHealthKey {
					require.True(t, exists)
					assert.Equal(t, tc.wantHealthValue, value)
				} else {
					assert.False(t, exists, "unexpected health marker %s", key)
				}
			}
		})
	}
