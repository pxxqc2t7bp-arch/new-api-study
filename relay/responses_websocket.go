...
type responsesWSSession struct {
	...
	// The serial request worker updates terminal IDs before publishing done under
	// stateMu; the target reader observes them only after that synchronization.
	// The remaining fields describe the established connection.
...
func (s *responsesWSSession) runCall(c *gin.Context, state *responsesWSCallState, create responsesWSCreateRequest) (apiErr *types.NewAPIError) {
	...
	if s.lockedChannelID != 0 {
		...
		for ; retry.GetRetry() <= maxRetries; retry.IncreaseRetry() {
			if retry.GetRetry() > 0 && orchestration.Enabled && time.Now().After(failoverDeadline) {
			...
			if err := s.registerChannelClose(channel.Id); err != nil {
			...
			if err := common.Unmarshal(incoming.body, &event); err != nil {
				...
				if event.Type == "error" {
					...
					if rejection.ResponseID != "" && rejection.ResponseID == s.lastResponseID ||
						rejection.StreamID != "" && rejection.StreamID == s.lastStreamID && rejection.StreamID != create.StreamID {
						continue
					}
					...
					if accepted {
						...
						s.lastStreamID = create.StreamID
						state.terminal, state.closeAfter = &incoming, ambiguous
						ConsumeResponsesQuota(c, info, accumulator.Finish())
						return nil
					}
					if rejection.StreamID == "" && rejection.ResponseID == "" {
						state.closeAfter = true
					}
					s.lastResponseID = rejection.ResponseID
					s.lastStreamID = create.StreamID
					if rejection.Error == nil {
	...
	s.workers.Go(func() {
		for {
			...
			if state := s.getCurrent(); state != nil {
				...
				case <-state.done:
					if !s.forwardIdleTargetMessage(incoming) {
						return
					}
				case <-s.ctx.Done():
					return
				}
			} else if !s.forwardIdleTargetMessage(incoming) {
				return
			}
			if err != nil {
				return
...
func (s *responsesWSSession) forwardIdleTargetMessage(incoming responsesWSMessage) bool {
	if incoming.err != nil {
		s.shutdown()
		return false
	}
	...
	if err := common.Unmarshal(incoming.body, &event); err == nil {
		if event.Type == "error" {
			if event.ResponseID != "" && event.ResponseID == s.lastResponseID ||
				event.StreamID != "" && event.StreamID == s.lastStreamID {
				return true
			}
			s.shutdown()
			return false
...
func (s *responsesWSSession) unregisterChannelClose() {
	s.connectionMu.Lock()
...
func (s *responsesWSSession) registerChannelClose(channelID int) error {
...
func selectResponsesWSChannel(c *gin.Context, modelName string, retryParam *service.RetryParam) (*appmodel.Channel, *types.NewAPIError) {
	ensureResponsesWSChannelFilter(c)
...
func responsesWSRetryTimes(retry *service.RetryParam, adaptive bool) int {
	ensureResponsesWSChannelFilter(retry.Ctx)
	if retry.TokenGroup == "auto" || adaptive {
		return service.AdaptiveRetryTimes(retry)
	}
	return common.RetryTimes
}

func ensureResponsesWSChannelFilter(c *gin.Context) {
