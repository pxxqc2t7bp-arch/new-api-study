...
export function ChannelMutateDrawer({
  ...
  const geminiLiveFields = currentType === CHANNEL_TYPE_GEMINI && (
    <FormField
      control={form.control}
      ...
      render={({ field }) => (
        <FormItem className='flex items-center justify-between gap-3 px-4 py-3'>
          <div className='space-y-0.5'>
            <FormLabel>{t('Enable Gemini Live')}</FormLabel>
            <FormDescription>
              {t('Allow native Gemini Live WebSocket sessions on this channel')}
            </FormDescription>
          </div>
          <FormControl>
            <Switch
