	...
	return func(c *gin.Context) {
		...
		if pin, found, overridden := constraints.ResolvedPin(); found {
			...
			if shouldSelectChannel {
				...
				if channel == nil {
					...
					if channel == nil {
						abortWithOpenAiMessage(c, http.StatusServiceUnavailable, noAvailableChannelMessage(c, usingGroup, modelRequest.Model), noAvailableChannelErrorCode(constraints))
						return
					}
				}
			}
		...
		if channel != nil {
			if ok, kind := model.ChannelSatisfiesFilters(channel, modelRequest.Model, constraints.Filters); !ok {
				...
				abortWithOpenAiMessage(c, http.StatusServiceUnavailable, noAvailableChannelMessage(c, common.GetContextKeyString(c, constant.ContextKeyUsingGroup), modelRequest.Model), channelFilterErrorCode(kind))
				return
			}
		}
...
func SetupContextForSelectedChannel(c *gin.Context, channel *model.Channel, modelName string) *types.NewAPIError {
	...
	c.Set("channel_priority", channel.GetPriority())
